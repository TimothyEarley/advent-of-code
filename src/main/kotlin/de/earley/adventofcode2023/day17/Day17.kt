...
object Day17 : BaseSolution<Grid<Int>, Long, Long>() {
	...
	override fun parseInput(input: Sequence<String>): Grid<Int> = input.toList().let { lines ->
		Grid(lines.first().length, lines.size, lines.flatMap { line -> line.map { it.digitToInt() } })
	}
