...
object Day19 : BaseSolution<List<Day19.Blueprint>, Int, Int>() {
	...
	override fun partTwo(data: List<Blueprint>): Int = data.take(3)
	...
	private fun mostGeodesCollected(blueprint: Blueprint, totalTime: Int): Int {
		return -generalAStar(
				...
				useClosed = true,
				neighbours = { nextStates(blueprint) }
		)
	}

	private fun State.nextStates(blueprint: Blueprint): Sequence<Pair<State, Int>> {
		...
		val buildOptions = buildList {
			...
			if (ore >= blueprint.geodeRobot.first && obsidian >= blueprint.geodeRobot.second) {
				add(copy(ore = ore - blueprint.geodeRobot.first, obsidian = obsidian - blueprint.geodeRobot.second, geodeRobot = geodeRobot + 1))
				if (geodeRobot == 0) canDoNothing = false
			} else {
				// Obsidian Robot
				if (ore >= blueprint.obsidianRobot.first && clay >= blueprint.obsidianRobot.second) {
					add(copy(ore = ore - blueprint.obsidianRobot.first, clay = clay - blueprint.obsidianRobot.second, obsidianRobot = obsidianRobot + 1))
					if (obsidianRobot == 0) canDoNothing = false
				}

				// Clay Robot
				if (canDoNothing && ore >= blueprint.clayRobot) {
